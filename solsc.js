...
sc_div.innerHTML = '<div class="vrs" style="width:100%; position:relative; padding:5px; color:#fff; font-size:12px;"><div class="vars"><label for="btc_price_try" style="color:#fff;">Btc Fiyat</label><input id="btc_price_try" type="number" style="color:#81b29a; background:transparent; border:1px solid #81b29a; border-radius:4px;" value="63237" /></div><div class="vars"><label for="stoploss" style="color:#fff;">STOP (BTC)</label><input id="stoploss" type="number" style="color:#81b29a; background:transparent; border:1px solid #81b29a; border-radius:4px;" value="0.00064" /></div><div class="vars"><label for="stopprofit" style="color:#fff;">KARDA DUR (BTC)</label><input id="stopprofit" type="number" style="color:#81b29a; background:transparent; border:1px solid #81b29a; border-radius:4px;" value="0.5" /></div><div class="vars"><button class="btn" onclick="startGame()">Start</button><button class="btn" onclick="stopGame()">Stop</button></div><div class="vars" style="font-size:18px" id="profit_display">Test </div></div>'
        ...
        if ($(event.currentTarget).is(':contains("lose")')) {
            ...
            if (currentProfit >= profitval) {
                ...
                if (currentLose <= stopval) {
                    ...
                    var txt;
                    let cmval = $('#double_your_btc_stake').val();
                    let msg = parseFloat(stopval).toFixed(8) + ' zararda durduruldu, ' + parseFloat(cmval).toFixed(8) + "Vuruş yapılsınmı?";
                    var r = confirm(msg);

                    if (r == true) {
                       
                       // $('#double_your_btc_stake').val(mx * 2);
                        setTimeout(function () {
                            console.log("vuruş")
                            $hiButton.trigger('click');
                        }, getRandomWait());
