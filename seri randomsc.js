// Variable to change:
var baseBetAmount = 0.00000020; // The base amount that you want to bet.
var betMultiplier = 2; // the multiplier value used.
var supportedLoseSequenceBetsLength = 8; // After 20 consecutive loose the script stops
var patternPlayPeriodInSeconds = 40; // The period of playing a single pattern.
var maxGain = 0.00000008; // The scripts stop when a maximum number of satoshi have benn earned.
var betsPatternsLengthInDecimal = [4];
var betPatternReversed = 0;
...
if ($(event.currentTarget).is(':contains("lose")')) {
	...
	console.log('Bets: ' + betsCounter + 'WIN:'+ggwin+' || Profit: ' + getProfit() + ' \n || Lose sequence: ' +
		currentLoseSequenceBetsCounter + ' Max is ' + supportedLoseSequenceBetsLength +' || Current bet: ' + bets[currentBetIndex] +
		' || Max lose consecutive: ' + maxLoseSequenceBetsCounter + ' || Max satoshi bet: ' + maxBet + ' || Wagered: ' + satWagered.toFixed(8));

	if (currentLoseSequenceBetsCounter-2 < supportedLoseSequenceBetsLength)  {
...
ggwin++;
	...
	betsCounter++;
	switch(currentLoseSequenceBetsCounter)
	{
		case 0:
		num0++;
		case 1:
	...
	if (( getProfit() >= maxGain) || (supportedLoseSequenceBetsLength-1 < currentLoseSequenceBetsCounter) || (ggwin==maxwin)){
		gameStopped = true;
	}

	console.log('Bets: ' + betsCounter + 'WIN:'+ggwin+' || Profit: ' + getProfit() + '\n || Lose sequence: ' +
		currentLoseSequenceBetsCounter + ' Max is ' + (supportedLoseSequenceBetsLength-1) +' || Current bet: ' + bets[currentBetIndex] +'\n'
		+' || Max lose consecutive: ' + maxLoseSequenceBetsCounter + ' || Max satoshi bet: ' + maxBet + ' \n|| Wagered: ' + satWagered.toFixed(8));
console.log('D0: '+num0+' D1: '+num1+' D2: '+num2+'\n D3:'+num3+' D4: '+num4+' D5: '+num5+'\n D6: '+num6+' D7:'+num7+' D8: '+num8+' .')
