function clickFree() {
	timeR = $('title').text();
	replText = timeR.replace("- FreeBitco.in - Her Saat Ücretsiz BTC Kazan!", "");
...
ChangeClass("double_your_btc_link", "Script");
ChangeClass("betting_link", "B."),
ChangeClass("golden_ticket_link", "Gold");
ChangeClass("earn_btc_link", "E");
ChangeClass("lottery_link", "Çekiliş");
ChangeClass("wager_promotion_link", "C.")
ChangeClass("refer_link", "Referans");
ChangeClass("dropbtn", "Daha");
ChangeClass("rewards_link", "Bonus");
ChangeClass("faq_link", "D.");
ChangeClass("edit_link", "Profil " + pot);
ChangeClass("stats_link", "Statu");
...
function summarize(amount0, wins, losses, delay, base, breaking) {
	...
	for(i = 0; i <= breaking; i++) {
		if(computed === false) {
			safetyness = safetyness + amount[i];
		}
	}
	//alert(safetyness);
	var i = 0;
	...
	for(i = 0; i <= breaking; i++) {
		summary.push(String(i + 1) + ". " + String(Math.round(amount[i] / 0.00000001)) + " " + String(wins[i]) + "/" + String(losses[i]) + " Avg:" + String(Math.round(wins[i] / (losses[i] + wins[i]) * 100)) + "%");
	};
	return summary.join('\r\n');
};
...
function summarize1(amount, wins, losses, delay, base, breaking, break_mult) {
	...
	//alert(amount1);
	for(i = 0; i < Number(delay) + Number(delay1); i++) {
		amount1.unshift(base1)
	}
	// var i=0;
	var i = 0;
	for(i = 0; i <= breaking2; i++) {
		if(computed === false) {
			safetyness = safetyness + amount1[i];
		}
	};
	//stop computing
	computed = true;
	...
	for(i = 0; i <= breaking2; i++) {
		summary.push(String(i + 1) + ". " + String(Math.round((Number(amount1[i]) / 0.00000001))) + " " + String(wins[i + breaking]) + "/" + String(losses[i + breaking]) + " Avg:" + String(Math.round(wins[i + breaking] / (losses[i + breaking] + wins[i + breaking]) * 100)) + "%")
	};
	return summary.join('\r\n');
};
	...
	if(stop_autobet == true) {
		...
		$.get('/cgi-bin/bet.pl?m=' + new_mode + '&client_seed=' + client_seed + '&jackpot=' + jackpot + '&stake=' + bet + '&multiplier=' + $("#double_your_btc_payout_multiplier").val() + '&rand=' + Math.random(), function(data) {
			...
			if(result[0] == "s1") {
				...
				if(result[1] == "l") {
					losses[loss_streak] = losses[loss_streak] + 1;
					loss_streak_new++;
					loss_streak++;
					win_streak = 0;
					if(highest_streak == loss_streak_new) {
						highest_occur++;
					};
					if(highest_streak < loss_streak_new) {
						highest_occur = 1;
						highest_won = 0;
						highest_streak = loss_streak_new
					};
					...
					if(autobet_lose_return_to_base == 1) {
						...
						//alert();
						if(loss_streak < Number(delay) + Number(delay2)) {
							new_bet_size = parseFloat(base);
							//Insert here the crazy bet
							var crazy = Math.floor(Math.random() * (2 - 1 + 1) + 1); //0 or 1
							if(crazy == 1) {
								new_bet_size = parseFloat(base) * Number(crazyno);
							}
						}
						//else if (balanse>0){new_bet_size=parseFloat(base_bet);alert("Staying at"+base_bet);}
						else {
							var new_bet_size = parseFloat(pattern[loss_streak - Number(delay2) - Number(delay)]).toFixed(8);
						}
						$("#double_your_btc_stake").val(new_bet_size);
					}
